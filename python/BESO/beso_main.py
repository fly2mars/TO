    ...
    if continue_iterations is False or i >= iterations_limit:
        ...
        if save_iteration_results and np.mod(float(i - 1), save_iteration_results) == 0:
            if "frd" in save_iteration_format:
                beso_lib.export_frd(path_export + "file" + str(i), nodes, Elements, elm_states, number_of_states)
            if "inp" in save_iteration_format:
                beso_lib.export_inp(path_export + "file" + str(i), nodes, Elements, elm_states, number_of_states)
...
#vis_tools.write_images_to_video(path_image, path_image + 'mash_curve.avi')
