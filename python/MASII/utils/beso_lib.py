...
##################################
# import load position from inp  #
##################################
def import_inp_load(file_name):
    '''
    import load position from inp
    '''
    try:
        ...
        msg = file_name + " file not found. Check your inputs."
        write_to_log(file_name, "ERROR: " + msg + "\n")
        assert False, msg
    ...
    for line in f:                
        if(line[:2] == '**'): continue
        if(line[0] == '\n'): continue
        if(read_cload is True and line[0] == '*' and line[1] != '*'):
            read_cload = False            
            
        if(line[:6] == '*CLOAD'):
            read_cload = True            
            
        if(read_cload is True and line[0] != '*'):    
            line_lists = line.split(',')            
            number = int(line_lists[0])
            f_orient = int(line_lists[1])
            f_magnitude = float(line_lists[2])
            
            load_nodes[number] = [number, f_orient, f_magnitude]      

    return load_nodes   


########################################
# import boundaries position from inp  #
########################################
def import_inp_boundary(file_name):
    '''
    import boundaries position from inp
    The freedom of constraints are not imported.
    '''
    try:
        f = open(file_name, "r")
    except IOError:
    ...
    read_nset = False
    boundary_set = []
    boundary_node_indexes = []
   
    # first scan: find BOUNDARY
    for line in f:                
        if(line[:2] == '**' or line[0] == '\n'): continue
        
        if(read_boundary is True and line[0] == '*' and line[1] != '*'):
            read_boundary = False            
            
        if(line[:9] == '*BOUNDARY'):
            read_boundary = True            
            
        if(read_boundary is True and line[0] != '*'):
            line_lists = line.split(',')      
            boundary_set.append(line_lists[0])
    
    # remove repeatd items
    boundary_set = list(set(boundary_set))
    
    # second scan: find boundary nodes
    f = open(file_name, "r")
    for line in f:        
        if(line[:2] == '**' or line[0] == '\n'): continue
        if(read_nset is True and line[0] == '*' and line[1] != '*'):
            read_nset = False           
        if(line[:5] == '*NSET'):            
            line_lists = line.split(',')
            key_value = line_lists[1].split('=')[1]
            key_value = key_value.replace('\n', '')
            if(key_value in boundary_set):
                read_nset = True                
        if(read_nset is True and line[0] != '*'):
            line_lists = line.split(',')      
            node_idx = int(line_lists[0])
            boundary_node_indexes.append(node_idx)        

    return boundary_node_indexes